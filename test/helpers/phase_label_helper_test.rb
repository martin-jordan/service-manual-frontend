...
class PhaseLabelHelperTest < ActionView::TestCase
  test "#surveymonkey_url includes a custom variable" do
    stub(:request, ActionDispatch::TestRequest.create('PATH_INFO' => '/service-manual/agile')) do
      assert_equal "https://www.smartsurvey.co.uk/s/HGLV5/?c=/service-manual/agile", surveymonkey_url
    end
  end
  ...
  test "#surveymonkey_url does not include a custom variable if the request path is nil" do
    stub(:request, ActionDispatch::TestRequest.create('PATH_INFO' => nil)) do
      assert_equal "https://www.smartsurvey.co.uk/s/HGLV5/", surveymonkey_url
    end
  end
end
